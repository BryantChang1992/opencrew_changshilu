...
1. **注册任务**：调用 `task_followup_register` 注册派单任务
   ```
   参数：
   - taskId: 任务ID
   - title: 任务标题
   - fromAgent: 你的Agent ID
   - toAgent: 接单Agent ID
   - channel: 目标频道ID
   - threadTs: Thread timestamp
   ```

2. **插件自动跟踪**：任务会被自动跟踪，无需手动管理

#### 汇报时（A必须做）

**每15分钟**（默认间隔，可配置）必须调用 `task_followup_report`：
...
- summary: 进展摘要
- blockedReason: 阻塞原因（如果blocked）
- estimatedCompletion: 预计完成时间（可选）
```

#### 超时机制（自动执行）

如果超过15分钟未汇报：
1. **自动提醒**：下次Agent启动时会收到超时警告
2. **自动升级**：每次超时升级计数+1
3. **达到上限**：升级3次后自动通知CEO介入

#### 查询状态

随时调用 `task_followup_status` 查询任务状态：
- 不提供参数：查询所有活动任务
- 提供taskId：查询指定任务
- 提供agentId：查询指定Agent的派单任务
...
~~2. **汇报内容**：当前状态、B 的产出摘要、下一步预期~~
~~3. **自动结束**：任务完成并输出 closeout 后，汇报自动结束~~

### 禁止行为（红线）

- ❌ **派单不注册**：派单后必须调用 `task_followup_register`
- ❌ **超时不汇报**：超过15分钟未调用 `task_followup_report`
- ❌ **虚假汇报**：进度必须真实，禁止伪造
